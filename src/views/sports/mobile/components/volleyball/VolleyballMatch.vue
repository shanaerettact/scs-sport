  ...
  <div class="match-card">
    ...
    <div class="flex items-center px-2.5 py-1.5 gap-2">
      ...
      <div class="flex items-center gap-1.5 text-[11px] text-subtle-light ml-auto" v-if="matchTimeType == 1">
        <span class="tabular-nums">{{ match.volleyballData?.set1HomeScore ?? '-' }}:{{ match.volleyballData?.set1AwayScore ?? '-' }}</span>
        <span class="opacity-30">|</span>
        <span class="tabular-nums">{{ match.volleyballData?.set2HomeScore ?? '-' }}:{{ match.volleyballData?.set2AwayScore ?? '-' }}</span>
        <span class="opacity-30">|</span>
        <span class="tabular-nums">{{ match.volleyballData?.set3HomeScore ?? '-' }}:{{ match.volleyballData?.set3AwayScore ?? '-' }}</span>
        <span class="opacity-30">|</span>
        <span class="tabular-nums font-semibold text-main">{{ match.volleyballData?.homeScore ?? '-' }}:{{ match.volleyballData?.awayScore ?? '-' }}</span>
      </div>
