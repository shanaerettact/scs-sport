  ...
  <div class="bg-surface rounded-sm p-2">
    <div class="flex items-center justify-between mb-2">
      ...
      <div class="flex items-center gap-2">
        <div class="flex items-center">
          <!-- 非滾球固定顯示賽事時間 -->
          <template v-if="matchTimeType != 1">{{ dayjs(match.matchTime).format('YYYY-MM-DD HH:mm') }}</template>
          <template v-else>
    ...
    <div class="flex gap-2 text-xs mt-1" v-if="matchTimeType == 1">
      ...
      <div class="flex items-center justify-center gap-0.5">
        <span class="text-center">{{ match.volleyballData?.set1HomeScore ?? '-' }}</span>
        <span>:</span>
        <span class="text-center">{{ match.volleyballData?.set1AwayScore ?? '-' }}</span>
      </div>

      <!-- 第2節 -->
      <div class="flex items-center justify-center gap-0.5">
        <span class="text-center">{{ match.volleyballData?.set2HomeScore ?? '-' }}</span>
        <span>:</span>
        <span class="text-center">{{ match.volleyballData?.set2AwayScore ?? '-' }}</span>
      </div>

      <!-- 第3節 -->
      <div class="flex items-center justify-center gap-0.5">
        <span class="text-center">{{ match.volleyballData?.set3HomeScore ?? '-' }}</span>
        <span>:</span>
        <span class="text-center">{{ match.volleyballData?.set3AwayScore ?? '-' }}</span>
      </div>

      <!-- 第4節 -->
      <div class="flex items-center justify-center gap-0.5">
        <span class="text-center">{{ match.volleyballData?.set4HomeScore ?? '-' }}</span>
        <span>:</span>
        <span class="text-center">{{ match.volleyballData?.set4AwayScore ?? '-' }}</span>
      </div>

      <!-- 第5節 -->
      <div class="flex items-center justify-center gap-0.5">
        <span class="text-center">{{ match.volleyballData?.set5HomeScore ?? '-' }}</span>
        <span>:</span>
        <span class="text-center">{{ match.volleyballData?.set5AwayScore ?? '-' }}</span>
      </div>

      <div class="flex items-center justify-center gap-0.5">
        <span class="text-center">{{ match.volleyballData?.homeScore ?? '-' }}</span>
        <span>:</span>
        <span class="text-center">{{ match.volleyballData?.awayScore ?? '-' }}</span>
      </div>
    </div>
  </div>
