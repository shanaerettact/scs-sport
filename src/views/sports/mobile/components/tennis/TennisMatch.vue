  ...
  <div class="match-card">
    ...
    <div class="flex items-center px-2.5 py-1.5 gap-2">
      ...
      <div class="flex items-center gap-1.5 text-[11px] text-subtle-light ml-auto" v-if="matchTimeType == 1">
        <span class="tabular-nums">{{ match.tennisData?.set1HomeGames ?? '-' }}:{{ match.tennisData?.set1AwayGames ?? '-' }}</span>
        <span class="opacity-30">|</span>
        <span class="tabular-nums">{{ match.tennisData?.set2HomeGames ?? '-' }}:{{ match.tennisData?.set2AwayGames ?? '-' }}</span>
        <span class="opacity-30">|</span>
        <span class="tabular-nums">{{ match.tennisData?.set3HomeGames ?? '-' }}:{{ match.tennisData?.set3AwayGames ?? '-' }}</span>
        <span class="opacity-30">|</span>
        <span class="tabular-nums font-semibold text-main">{{ match.tennisData?.homeSets ?? '-' }}:{{ match.tennisData?.awaySets ?? '-' }}</span>
      </div>
