<template>
  <div class="bg-surface rounded-sm p-2">
    ...
    <div class="flex gap-1 text-xs mt-1" v-if="matchTimeType == 1">
      <div class="flex items-center justify-center gap-0.5">
        <span class="text-center">{{ match.baseballData?.fullInclExtraHomeScore ?? '-' }}</span>
        <span>:</span>
        <span class="text-center">{{ match.baseballData?.fullInclExtraAwayScore ?? '-' }}</span>
      </div>
    </div>
  </div>
