<template>
  <div class="relative z-[3000]">
    <div class="rounded-md bg-surface-soft p-1">
      <div class="flex items-center justify-center cursor-pointer" @click="handleFloating()">
        <n-icon size="28" v-html="SlideText20Regular" v-if="betStore.floatBetting == false"></n-icon>
        <n-icon size="28" v-html="CloseCircleOutline" v-if="betStore.floatBetting == true"></n-icon>
      </div>
    </div>

    <!-- 投注單內容 -->
    <div class="w-[360px] bg-bet-panel rounded-md absolute top-0 right-12" v-if="betStore.floatBetting">
      <div class="betting-header">
        <div class="flex items-center rounded-t-md bg-primary text-black gap-3 px-4 py-1">
          <n-icon size="48" v-html="SlideText20Regular"></n-icon>
          <span class="text-lg font-bold">{{ $t('label.betSlip') }}</span>
        </div>
...
import { NButton, NIcon, NInputNumber, NRadio, NRadioGroup, NTabPane, NTabs, useMessage } from 'naive-ui';
import { computed, reactive, ref, watch } from 'vue';
import Close from "@/assets/ionIcons5/Close.svg?raw";
import CloseCircleOutline from "@/assets/ionIcons5/CloseCircleOutline.svg?raw";
...
const handleFloating = () => {
  betStore.floatBetting = !betStore.floatBetting;
  betStore.isComboOrder = false;
};
