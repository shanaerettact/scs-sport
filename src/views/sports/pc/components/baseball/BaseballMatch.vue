<template>
  <div class="grid border-b border-slate-300" :class="{ 'border-t': matchIndex === 0, 'grid-cols-[120px_1fr_repeat(4,_minmax(0,1fr))]': matchStore.collapseSide, 'grid-cols-[120px_1fr_repeat(6,_minmax(0,1fr))]': !matchStore.collapseSide }">
    <!-- 左側比賽資訊 -->
    ...
    <div class="col-span-2 border-x border-slate-300 bg-surface">
      <!-- 表頭 -->
      <div class="min-h-8 grid grid-cols-[1fr_30px] border-b border-slate-300 text-center font-semibold px-2 py-1" v-if="matchIndex === 0">
      ...
      <div class="grid grid-rows-3 min-h-28 px-2 py-1 gap-2">
        <div class="grid items-center" :class="{ 'grid-cols-[1fr_30px]': matchTimeType == 1 }">
          ...
          <template v-if="matchTimeType == 1">
            <div class="text-center">{{ match.baseballData?.fullInclExtraHomeScore ?? '-' }}</div>
          </template>
        </div>
        ...
        <div class="grid items-center" :class="{ 'grid-cols-[1fr_30px]': matchTimeType == 1 }">
          ...
          <template v-if="matchTimeType == 1">
            <div class="text-center">{{ match.baseballData?.fullInclExtraAwayScore ?? '-' }}</div>
          </template>
        </div>
...
const hiddenMarket: any[] = ['3005', '3006'];
